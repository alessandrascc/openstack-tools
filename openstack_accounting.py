...
class openstack_accounting( object ):
  ...
  def prepareDict( self, dbName, tableName, columnNames):
    ...
    for row in mysql_data:
      ...
      stuck_machine = 0
      is_active = 0
#modification added for quotas purposes - Alessandra
      if len(columnNames) == 6 and (row[5] == 'deleting' or row[5] == 'building'):
        stuck_machine = 1
      if row[2] != None:
      ...
      if not user_id in return_data:
        if len(columnNames) == 6:
          return_data[user_id] = [eff_size, is_active, one, stuck_machine]
#modification added for quotas purposes - Alessandra
        else:
...
#modification added for quotas purposes - Alessandra
        if bool(stuck_machine) :
          return_data[user_id][3] += one
    return return_data
...
def main():
  ...
  instance_mapping = reader.prepareDict(reader.getNovadb(), "instances", ["user_id","created_at", "deleted_at", "vcpus","deleted"])
  volume_mapping = reader.prepareDict(reader.getCinderdb(), "volumes", ["user_id","created_at", "deleted_at", "size", "deleted"])
